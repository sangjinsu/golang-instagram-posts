...
원격 실행 환경에서 브라우저 경로를 오버라이드하려면
PLAYWRIGHT_CHROMIUM_EXECUTABLE_PATH 환경변수를 설정한다:
...
def export_slides(content_id, slide_filter=None):
    ...
    # 환경에 따라 Playwright 브라우저 경로 오버라이드 (원격 실행 환경 대응)
    ...
    with sync_playwright() as p:
        ...
        page.goto(f'file://{abs_html}', wait_until='networkidle', timeout=NAV_TIMEOUT_MS)
